...
const recycling_helpers_2 = require("./recycling-helpers");
...
const autoTranslate = (args) => {
    ...
    const translationMemories = translationMemoryPaths.map(path => recycling_helpers_1.deserializeTranslationMemory(fs_1.readFileSync(path).toString()));
    const translated = recycling_helpers_2.recycleTranslation(resources, translationMemories);
